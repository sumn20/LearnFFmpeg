...
void VideoDecoder::initFFMpegContext() {
	...
	//4.0已经过时,将不再推荐使用，而且是非必需的，因此直接注释掉即可。
	//  av_register_all();
	//对FFmpeg log初始化
...
//	LOGCATI("openInput [%s] waste TimeMills is %d", requestHeader->getURI(), (int )(currentTimeMills() - startTimeMills));
	...
	if (errorCode > 0) {
		if (isNeedBuriedPoint) {
			...
			LOGCATI("successOpen is %f", buriedPoint.successOpen);
		}
		...
		if (isNeedBuriedPoint) {
			...
			LOGCATI("failOpen is %f", buriedPoint.failOpen);
		}
		return errorCode;
	}
...
#define FAIL(errmsg) do {                                         \
    ...
    switch (avctx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            LOGCATI("check AVMEDIA_TYPE_AUDIO");
        ...
        case AVMEDIA_TYPE_VIDEO:
            LOGCATI("check AVMEDIA_TYPE_VIDEO");
            if (!avctx->width)
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            LOGCATI("check AVMEDIA_TYPE_SUBTITLE");
            if (avctx->codec_id == AV_CODEC_ID_HDMV_PGS_SUBTITLE && !avctx->width)
                FAIL("unspecified size");
            break;
        case AVMEDIA_TYPE_DATA:
            LOGCATI("check AVMEDIA_TYPE_DATA");
...
//	LOGCATI("VideoDecoder::openInput");
	...
	if ((openInputErrCode = this->openFormatInput(videoSourceURI)) != 0) {
		LOGCATI("Video decoder open input file failed... videoSourceURI is %s openInputErr is %s", videoSourceURI, av_err2str(openInputErrCode));
		return -1;
	}
	this->initAnalyzeDurationAndProbesize(max_analyze_durations, analyzeDurationSize, probesize, fpsProbeSizeConfigured);
//	LOGCATI("pFormatCtx->max_analyze_duration is %d", pFormatCtx->max_analyze_duration);
//	LOGCATI("pFormatCtx->probesize is %d", pFormatCtx->probesize);
	//获取文件中的流信息，此函数会读取packet，并确定文件中所有的流信息  设置pFormatCtx->streams指向文件中的流，但此函数并不会改变文件指针，读取的packet会给后面的解码进行处理
	if (avformat_find_stream_info(pFormatCtx, NULL) < 0) {
//		avformat_close_input(&pFormatCtx);
		LOGCATI("Video decoder Stream info not found...");
		return -1;
	}
	is_eof = false;
...
//	av_dump_format(pFormatCtx, -1, videoSourceURI, 0);
	if (this->isNeedRetry()) {
		if (isNeedBuriedPoint) {
			...
			LOGCATI("retryTime is %f", retryTime);
		}
		avformat_close_input(&pFormatCtx);
		avformat_free_context(pFormatCtx);
		return openInput();
	} else {
		LOGCATI("retry finish");
...
int VideoDecoder::openVideoStream() {
	...
	LOGCATI("videoStreams size is %d", videoStreams->size());
	std::list<int>::iterator i;
	for (i = videoStreams->begin(); i != videoStreams->end(); ++i) {
		int iStream = *i;
		LOGCATI("videoStreamIndex is %d", iStream);
...
int VideoDecoder::openVideoStream(int streamIndex) {
	LOGCATI("VideoDecoder::openVideoStream");
...
void VideoDecoder::flushAudioFrames(AVPacket* packet, std::list<MovieFrame*> * result,
		float minDuration, int* decodeVideoErrorState) {
	if (audioCodecCtx->codec->capabilities & AV_CODEC_CAP_DELAY) {
