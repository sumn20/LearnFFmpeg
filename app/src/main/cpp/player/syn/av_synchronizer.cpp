	...
	while (true) {
		...
		if(ret > 0){
			...
			if (delta < (0 - syncMaxTimeDiff)) {
				//视频比音频快了好多,我们还是渲染上一帧
//				LOGCATI("视频比音频快了好多,我们还是渲染上一帧 moviePosition is %.4f texture->position is %.4f", moviePosition, texture->position);
				texture = NULL;
				break;
			}
...
//				LOGCATI("视频比音频慢了好多,我们需要继续从queue拿到合适的帧 moviePosition is %.4f texture->position is %.4f", moviePosition, texture->position);
				continue;
			} else {
				break;
			}
...
//	LOGCATI("enter AVSynchronizer::fillAudioData... buffered is %d", buffered);
	this->signalDecodeThread();
	this->checkPlayState();
	if(buffered) {
//		LOGCATI("fillAudioData if(buffered) circleFrameTextureQueue->getValidSize() %d", circleFrameTextureQueue->getValidSize());
...
//			LOGCATI("audioFrameQueue->size() is %d", count);
			if (count > 0) {
		...
		if (NULL != currentAudioFrame) {
			...
			LOGCATI("fillAudioData NULL == currentAudioFrame");
			memset(outData, 0, bufferSize);
			bufferSize = 0;
			break;
		}
	}
//	LOGCATI("leave AVSynchronizer::fillAudioData...");
...
//	LOGCATI("decoder->getVideoFPS() is %.3f maxBufferedDuration is %.3f", fps, maxBufferedDuration);
...
bool AVSynchronizer::init(DecoderRequestHeader *requestHeader, JavaVM *g_jvm, jobject obj,float minBufferedDuration, float maxBufferedDuration) {
	LOGCATI("Enter AVSynchronizer::init");
	currentAudioFrame = NULL;
	currentAudioFramePos = 0;
	isCompleted = false;
	moviePosition = 0;
	...
	if (initCode < 0 || isDestroyed) {
		LOGCATI("VideoDecoder decode file fail...");
		closeDecoder();
		return false;
	}
	if (!decoder->isSubscribed() || isDestroyed) {
		LOGCATI("decoder has not Subscribed...");
		closeDecoder();
		return false;
	}
	...
	LOGCATI("Leave AVSynchronizer::init");
	return true;
}
...
//				LOGCATI("audioFrameQueue->push(audioFrame) position is %.4f", audioFrame->position);
				bufferedDuration += audioFrame->duration;
			}
		}
		pthread_mutex_unlock(&audioFrameQueueMutex);
	}
//	LOGCATI("bufferDuration is %.3f thresholdDuration is %.3f buffered is %d", bufferedDuration, thresholdDuration, buffered);
...
void AVSynchronizer::decode(){
	...
	// 先假定seek之前已经pause了
	//			LOGCATI("before pthread_cond_wait");
	...
	//			LOGCATI("after pthread_cond_wait");
...
void AVSynchronizer::processDecodingFrame(bool& good, float duration){
	...
	if (NULL != frames) {
		if (!frames->empty()) {
			...
			LOGCATI("frames is empty %d", (int )good);
		}
		delete frames;
	} else {
		LOGCATI("why frames is NULL tell me why?");
	}
}
