...
TextureFrameUploader::TextureFrameUploader() :
	...
	LOGCATI("TextureFrameUploader instance created");
	eglCore = NULL;
	textureFrame = NULL;
	outputTexId = -1;
	isInitial = false;
...
TextureFrameUploader::~TextureFrameUploader() {
	LOGCATI("TextureFrameUploader instance destroyed");
...
bool TextureFrameUploader::start(int videoWidth, int videoHeight, int degress) {
	LOGCATI("Creating TextureFrameUploader thread");
...
//	LOGCATI("enter TextureFrameUploader::signalFrameAvailable");
...
void TextureFrameUploader::stop() {
	LOGCATI("Stopping TextureFrameUploader Render thread");
	...
	LOGCATI("TextureFrameUploader Render thread stopped");
}
...
void TextureFrameUploader::renderLoop() {
	...
	while (renderingEnabled) {
		...
		switch (_msg) {
		case MSG_WINDOW_SET:
			LOGCATI("receive msg MSG_WINDOW_SET");
			isInitial = initialize();
			break;
		case MSG_RENDER_LOOP_EXIT:
			LOGCATI("receive msg MSG_RENDER_LOOP_EXIT");
	...
	LOGCATI("Render loop exits");
	return;
}

bool TextureFrameUploader::initialize() {
	eglCore = new EGLCore();
	LOGCATI("TextureFrameUploader use sharecontext");
	eglCore->initWithSharedContext();
	LOGCATI("after TextureFrameUploader use sharecontext");
	...
	eglCore->makeCurrent(copyTexSurface);
	LOGCATI("leave TextureFrameUploader::initialize");

	return true;
}

void TextureFrameUploader::destroy() {
	LOGCATI("dealloc eglCore ...");
	if (mUploaderCallback)
