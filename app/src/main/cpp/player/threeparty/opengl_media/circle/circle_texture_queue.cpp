...
void CircleFrameTextureQueue::init(int width, int height, int queueSizeParam){
	LOGCATI("enter CircleFrameTextureQueue::init width is %d height is %d queueSize is %d", width, height, queueSizeParam);
	if(queueSizeParam < 5){
		LOGCATI("circle video packet queue min size is 5");
		return;
	}
...
//	LOGCATI("leave CircleFrameTextureQueue::init");
...
//	LOGCATI("buildFrameTexture:width is %d height is %d texId is %d", width, height, frameTexture->texId);
	return frameTexture;
}
...
//	LOGCATI("enter CircleFrameTextureQueue::lockPushCursorFrameTexture");
	if (mAbortRequest) {
		return NULL;
	}
	int getLockCode = pthread_mutex_lock(&mLock);
...
//	LOGCATI("enter CircleFrameTextureQueue::unLockPushCursorFrameTexture");
	if (mAbortRequest) {
		return;
	}
...
//	LOGCATI("enter CircleFrameTextureQueue::clear");
	pthread_mutex_lock(&mLock);
	isAvailable = false;
	pullCursor = head;
	pushCursor = head;
	pthread_mutex_unlock(&mLock);
//	LOGCATI("leave CircleFrameTextureQueue::clear");
}
...
//	FrameTextureNode* beginCursor = pullCursor;
//	FrameTextureNode* endCursor = pushCursor;
//	LOGCATI("pullCursor texId : %d pushCursor texId : %d", pullCursor->texture->texId, pushCursor->texture->texId);
...
bool CircleFrameTextureQueue::checkGlError(const char* op) {
	...
	for (error = glGetError(); error; error = glGetError()) {
		LOGCATI("error::after %s() glError (0x%x)\n", op, error);
		return true;
	}
	return false;
}
...
//	LOGCATI("%s ~CircleFrameTextureQueue ....", queueName);
	flush();
...
//	LOGCATI("\n %s flush .... and this time the queue size is %d \n", queueName, queueSize);
	pthread_mutex_lock(&mLock);
//	LOGCATI("success get a lock in CircleFrameTextureQueue flush method...");
