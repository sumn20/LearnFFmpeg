...
#define  LOG_TAG "video_gl_surface_render"
...
bool VideoGLSurfaceRender::init(int width, int height) {
    ...
    if (!mGLProgId) {
        LOGCATE("Could not create program.");
        return false;
    }
...
void VideoGLSurfaceRender::renderToView(GLuint texID, int screenWidth, int screenHeight) {
    ...
    if (!mIsInitialized) {
        LOGCATE("ViewRenderEffect::renderEffect effect not initialized!");
        return;
    }
...
float VideoGLSurfaceRender::calcCropRatio(int screenWidth, int screenHeight, int texWidth,
                                          int texHeight) {
...
void VideoGLSurfaceRender::renderToViewWithAutoFit(GLuint texID, int screenWidth, int screenHeight,
...
void VideoGLSurfaceRender::renderToViewWithAutoFill(GLuint texID, int screenWidth, int screenHeight,
                                                    int texWidth, int texHeight) {
    glViewport(0, 0, screenWidth, screenHeight);
...
void VideoGLSurfaceRender::renderToAutoFitTexture(GLuint inputTexId, int width, int height,
                                                  GLuint outputTexId) {
...
//		LOGCATI("xOffset is %.3f", xOffset);
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        LOGCATI("failed to make complete framebuffer object %x", status);
    }
...
void VideoGLSurfaceRender::renderToCroppedTexture(GLuint inputTexId, GLuint outputTexId,
    ...
    LOGCATI("originalWidth is %d originalHeight is %d _backingWidth is %d _backingHeight is %d", originalWidth, originalHeight, _backingWidth, _backingHeight);
    glViewport(_backingLeft, _backingTop, _backingWidth, _backingHeight);
